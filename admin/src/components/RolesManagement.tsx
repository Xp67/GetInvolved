...
function RolesManagement({ userPermissions = [] }: RolesManagementProps) {
    ...
    const handleOpen = (role: any = null) => {
        if (role) {
            ...
            setFormData({
                ...
                permission_ids: role.permissions_details.map((p: any) => p.id)
            });
        } else {
            setEditingRole(null);
        ...
        <Box>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', mb: 3 }}>
                {hasPermission('roles.create') && (
                    <Button variant="contained" color="primary" onClick={() => handleOpen()} sx={{ textTransform: 'none', borderRadius: 2 }}>
                        Crea Ruolo
                    </Button>
                )}
            </Box>
                            ...
                            <TableRow key={role.id}>
                                ...
                                <TableCell>
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                        {role.permissions_details.slice(0, 3).map((p: any) => (
                                            <Chip key={p.id} label={p.name} size="small" variant="outlined" />
                                        ))}
                                        {role.permissions_details.length > 3 && (
                                            <Tooltip title={role.permissions_details.slice(3).map((p: any) => p.name).join(', ')}>
                                ...
                                <TableCell align="right">
                                    {hasPermission('roles.edit') && (
                                        <IconButton onClick={() => handleOpen(role)} disabled={role.name === 'Super Admin'} color="primary">
                                            <EditIcon />
                                        </IconButton>
                                    )}
            ...
            <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
                ...
                <form onSubmit={handleSubmit}>
                    <DialogContent dividers>
                        <TextField margin="normal" required fullWidth label="Nome Ruolo" name="name" value={formData.name} onChange={handleInputChange} disabled={editingRole && !editingRole.is_deletable} />
                        <TextField margin="normal" fullWidth label="Descrizione" name="description" multiline rows={2} value={formData.description} onChange={handleInputChange} />
                        <Typography variant="h6" sx={{ mt: 3, mb: 1 }}>Permessi</Typography>
                        <Divider sx={{ mb: 2 }} />
                        {categories.map((cat: any) => (
                            <Box key={cat.id} sx={{ mb: 3 }}>
                                <Typography variant="subtitle1" fontWeight="bold" color="primary" gutterBottom>{cat.name}</Typography>
                                <Grid container spacing={1}>
                                    {cat.permissions.map((perm: any) => (
                                        <Grid size={{ xs: 12, sm: 6, md: 4 }} key={perm.id}>
                                            <FormControlLabel
                                                control={<Checkbox checked={formData.permission_ids.includes(perm.id)} onChange={() => handlePermissionToggle(perm.id)} />}
                                                label={perm.name}
                                            />
                                        </Grid>
                                    ))}
                    ...
                    <DialogActions sx={{ p: 2 }}>
                        ...
                        <Button type="submit" variant="contained" sx={{ textTransform: 'none' }}>Salva</Button>
                    </DialogActions>
                </form>
            </Dialog>
        </Box>
