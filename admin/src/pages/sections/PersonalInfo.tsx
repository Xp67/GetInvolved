import { Box, Typography, Grid, Tooltip, Avatar, InputAdornment, Button, Paper } from "@mui/material";
...
export default function PersonalInfo({
    profile, setProfile, setMessage, setOpenSnackbar, isHoveringAvatar, setIsHoveringAvatar, fileInputRef, saving, setSaving, isMobile
}: any) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        api.patch("/api/user/profile/", formData, { headers: { "Content-Type": "multipart/form-data" } }).then((res: any) => {
            ...
            <Grid container spacing={isMobile ? 4 : 6} justifyContent={isMobile ? "center" : "flex-start"}>
                ...
                <Grid size={{ xs: 12, lg: 8 }}>
                    <Grid container spacing={3}>
                        ...
                        <Grid size={{ xs: 12, sm: 6 }}>
                            <AppTextField fullWidth label="Email" disabled value={profile.email} helperText="L'email non può essere modificata."
                                InputProps={{ startAdornment: <InputAdornment position="start"><EmailIcon color="disabled" fontSize="small" /></InputAdornment> }} />
                        </Grid>
                        <Grid size={12}>
                            <AppTextField fullWidth label="Numero di Telefono" value={profile.phone_number} onChange={(e: any) => setProfile({ ...profile, phone_number: e.target.value })}
                                InputProps={{ startAdornment: <InputAdornment position="start"><PhoneIcon color="action" fontSize="small" /></InputAdornment> }} />
                        </Grid>
                        <Grid size={12}>
                            <AppTextField fullWidth multiline minRows={1} maxRows={10} label="Bio" value={profile.bio} onChange={(e: any) => setProfile({ ...profile, bio: e.target.value })}
