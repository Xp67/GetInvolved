...
function EventEdit() {
  ...
  const startScanner = async () => {
    setScannerOpen(true);
    // Give time for the "reader" div to mount in the Dialog
    setTimeout(async () => {
      try {
        ...
        try {
          ...
          console.warn("Failed to start with environment mode, trying any camera", err);
          await scanner.start(
        ...
        setSnackbar({
          open: true,
          message: "Impossibile avviare la fotocamera. Assicurati di aver concesso i permessi e di non avere altre app che la usano.",
          severity: "error"
        });
    ...
    <Container maxWidth={false} sx={{ mt: 4, mb: 4, px: { xs: 2, md: 4 } }}>
      ...
      <Grid container spacing={3} alignItems="stretch">
        ...
        <Grid item xs={12} md={3}>
          <Paper sx={{ borderRadius: 2, overflow: 'hidden', height: '100%' }}>
            <Tabs
              ...
              sx={{ borderRight: 1, borderColor: 'divider', minHeight: 200, height: '100%' }}
            >
        ...
        <Grid item xs={12} md={9} >
          <Paper sx={{ p: { xs: 2, sm: 4 }, borderRadius: 2, minHeight: 400, height: '100%' }}>
            {activeTab === 0 && (
      ...
      <Dialog open={scannerOpen} onClose={stopScanner} fullWidth maxWidth="xs">
        ...
        <DialogContent sx={{ pb: 3 }}>
          <Box id="reader" sx={{ width: '100%', minHeight: '300px', bgcolor: '#000', borderRadius: 1, overflow: 'hidden' }}></Box>
